...
# Sample an initial values for parameters -----------------------------------------------------------------------
...
# Calculate expect values for different spawner-recruit models --------------------------------

expect_Ricker <- ssb/phi0*exp(h*(1-ssb/(R0*phi0)))
expect_RickerAlt <- R0*exp(h * (1.0 - ssb /SB0)) #Check equation with Matthew
expect_BevertonHolt <- (4 * h * R0 * ssb)/(SB0 * (1 - h) + ssb * (5 * h -1))
expect_BevertonHoltAlt <- (0.8*R0*h*ssb)/(0.2*R0*phi0*(1-h) + ssb*(h-0.2)) #Check equation with Matthew
