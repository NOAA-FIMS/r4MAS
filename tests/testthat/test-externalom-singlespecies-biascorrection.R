...
# Test RMAS: check model inputs -------------------------------------------------------------------
# Initialize RMAS module, input data, and model settings
# Fix model inputs and compare model outputs with true values

# Check model inputs ----------------------------------------------------------------------

test_that("External OM test: singlespecies with bias correction in recruitment",{

  # Load module
  r4mas <- Module("rmas", dyn.load(dll_path))
  test_path <- file.path(test_example_path, "externalom_example")
  load(file.path(test_path, "singlespecies_biascorrection.RData"))
  ...
  # check if relative error of MAS is less than 0.2
  for(i in 1:length(mas)){
    x <- mas[[i]]
    y <- om[[i]]
    for(j in 1:length(x)){
      expect_lt(abs(x[j] - y[j])/y[j], 0.20)
    }
  }
