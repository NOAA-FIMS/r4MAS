# Compute NLL for robust dirichlet-multinomial model of age composition data
...
for (a in 1:Amax)
{
  P_pred[a] <- (P_pred[a]+epsilon)
  tmp1 <- tmp1 + P_pred[a]
}
for (a in 1:Amax)
{
  P_pred[a] <- P_pred[a]/tmp1
}
...
{
  print(NLL)
  print(lgamma(N*P_obs[a]+Theta*N*P_pred[a]))
  print(lgamma(Theta*N*P_pred[a]))
...
# Output Data
print("Robust Dirichlet-Multinomial NLL Case 1")
...
# Output Data
print("Robust Dirichlet-Multinomial NLL Case 2")
print("____________________________________________________________________")
print("Number of age categories")
print(Amax)
print("Input sample size fraction parameter")
