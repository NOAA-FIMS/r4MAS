...
model_names = c("OM", "AMAK", "ASAP", "BAM", "SS", "MAS")
figure_number <- 10
...
for(om_sim in 1:om_sim_num){
  ...
  ostr<- paste(c("MAS/mas_s1_em",i), collapse = "")
  ...
  mas_msy[, om_sim] <-NA
  mas_fmsy[, om_sim] <- NA
  mas_ssbmsy[, om_sim] <- NA
  mas_fratio[, om_sim] <- NA
  mas_ssbratio[, om_sim] <- NA
  mas_agecomp[[om_sim]] <- NA
}
...
save(om_list, amak_list, asap_list, bam_list, ss_list, mas_list, file=file.path(maindir, "Outputs", "om_em_output.RData"))
