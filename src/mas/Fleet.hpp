...
struct Fleet: mas::ModelObject<REAL_T> {
	...
	int fishery_length_comp_likelihood_component_id = -999;
	atl::intrusive_ptr<mas::NLLFunctor<REAL_T> > fishery_length_comp_likelihood_component;
	...
	std::unordered_map<int,
			std::unordered_map<int,
					atl::intrusive_ptr<mas::SelectivityBase<REAL_T> > > > season_area_selectivity;

	std::unordered_map<int, std::unordered_map<int, int> > area_season_selectivity_ids;
	std::unordered_map<int,
			std::unordered_map<int,
					atl::intrusive_ptr<mas::SelectivityBase<REAL_T> > > > area_season_selectivity;
	...
	std::unordered_map<int,
			std::unordered_map<int,
					atl::intrusive_ptr<mas::FishingMortality<REAL_T> > > > area_season_fishing_mortality;
	...
	std::unordered_map<int,
			std::unordered_map<int,
					atl::intrusive_ptr<mas::FishingMortality<REAL_T> > > > season_area_fishing_mortality;
	...
	typedef typename std::unordered_map<int,
			atl::intrusive_ptr<mas::SelectivityBase<REAL_T> > >::iterator area_sectivity_iterator;
	...
	typedef typename std::unordered_map<int,
			std::unordered_map<int,
					atl::intrusive_ptr<mas::SelectivityBase<REAL_T> > > >::iterator season_area_selectivity_iterator;
	typedef typename std::unordered_map<int,
			std::unordered_map<int,
					atl::intrusive_ptr<mas::FishingMortality<REAL_T> > > >::iterator season_area_fishing_mortality_iterator;
	typedef typename std::unordered_map<int,
			atl::intrusive_ptr<mas::FishingMortality<REAL_T> > >::iterator area_fishing_mortality_iterator;
	typedef typename std::unordered_map<int,
			atl::intrusive_ptr<mas::FishingMortality<REAL_T> > >::iterator delta_method_fishing_mortality_iterator;
	...
	std::vector<variable> catch_length_at_age;
	...
	std::vector<variable> catch_length_at_age_males;
	...
	std::vector<variable> catch_length_at_age_females;
	std::vector<REAL_T> fleet_population_total_interactions; //number of sub-populations impacted by this fleet
	...
	void Initialize(size_t years, size_t seasons, size_t ages) {
		...
		catch_length_at_age_males.resize(years * seasons * ages);
		...
		catch_length_at_age_females.resize(years * seasons * ages);
		...
		this->catch_biomass_at_age_both[mas::FEMALE] =
				&(this->catch_biomass_at_age_females);
		this->catch_biomass_at_age_both[mas::MALE] =
				&(this->catch_biomass_at_age_males);
	}
				...
				case mas::FEMALE:
					...
					this->nll_components.push_back(
							mas::NLLComponent<REAL_T>(
									...
									this->data_objects[i],
									this->fishery_length_comp_likelihood_component));
					break;
	...
	void Prepare() {
		...
		for (int i = 0; i < this->catch_at_age.size(); i++) {
			...
			mas::VariableTrait<REAL_T>::SetValue(catch_biomass_at_age[i],
					static_cast<REAL_T>(0.0));
			mas::VariableTrait<REAL_T>::SetValue(catch_length_at_age[i],
					static_cast<REAL_T>(0.0));
			mas::VariableTrait<REAL_T>::SetValue(catch_proportion_at_age[i],
					static_cast<REAL_T>(0.0));
			mas::VariableTrait<REAL_T>::SetValue(
					catch_biomass_proportion_at_age[i],
					static_cast<REAL_T>(0.0));

			mas::VariableTrait<REAL_T>::SetValue(catch_at_age_males[i],
					static_cast<REAL_T>(0.0));
			mas::VariableTrait<REAL_T>::SetValue(catch_biomass_at_age_males[i],
					static_cast<REAL_T>(0.0));
			mas::VariableTrait<REAL_T>::SetValue(catch_length_at_age_males[i],
					static_cast<REAL_T>(0.0));
			mas::VariableTrait<REAL_T>::SetValue(
					catch_proportion_at_age_males[i], static_cast<REAL_T>(0.0));
			mas::VariableTrait<REAL_T>::SetValue(
					catch_biomass_proportion_at_age_males[i],
					static_cast<REAL_T>(0.0));

			mas::VariableTrait<REAL_T>::SetValue(catch_at_age_females[i],
					static_cast<REAL_T>(0.0));
			mas::VariableTrait<REAL_T>::SetValue(
					catch_biomass_at_age_females[i], static_cast<REAL_T>(0.0));
			mas::VariableTrait<REAL_T>::SetValue(catch_length_at_age_females[i],
					static_cast<REAL_T>(0.0));
			mas::VariableTrait<REAL_T>::SetValue(
					catch_proportion_at_age_females[i],
					static_cast<REAL_T>(0.0));
			mas::VariableTrait<REAL_T>::SetValue(
					catch_biomass_proportion_at_age_females[i],
					static_cast<REAL_T>(0.0));
		}

		for (int i = 0; i < this->catch_biomass_total.size(); i++) {
			fleet_population_total_interactions[i] = static_cast<REAL_T>(0.0);
			mas::VariableTrait<REAL_T>::SetValue(catch_abundance[i],
					static_cast<REAL_T>(0.0));
			mas::VariableTrait<REAL_T>::SetValue(catch_abundance_males[i],
					static_cast<REAL_T>(0.0));
			mas::VariableTrait<REAL_T>::SetValue(catch_abundance_females[i],
					static_cast<REAL_T>(0.0));
			mas::VariableTrait<REAL_T>::SetValue(catch_biomass_total[i],
					static_cast<REAL_T>(0.0));
			mas::VariableTrait<REAL_T>::SetValue(catch_biomass_total_males[i],
					static_cast<REAL_T>(0.0));
			mas::VariableTrait<REAL_T>::SetValue(catch_biomass_total_females[i],
					static_cast<REAL_T>(0.0));
		}
		...
		for (int y = 0; y < this->years; y++) {
			if (this->active_years[y]) {
				for (int s = 0; s < this->seasons; s++) {
					...
					variable &total_c_b = this->catch_biomass_total[y
							* this->seasons + s];
					variable &total_c_b_males =
							this->catch_biomass_total_males[y * this->seasons
									+ s];
					variable &total_c_b_females =
							this->catch_biomass_total_females[y * this->seasons
									+ s];
					...
					for (int a = 0; a < this->ages; a++) {
						index = y * this->seasons * this->ages
								+ (s * this->ages) + a;
					...
					for (int a = 0; a < this->ages; a++) {
						...
						catch_proportion_at_age[index] = catch_at_age[index]
								/ total_c;
						catch_proportion_at_age_males[index] =
								catch_at_age_males[index] / total_c_males;
						catch_proportion_at_age_females[index] =
								catch_at_age_females[index] / total_c_females;

						catch_biomass_proportion_at_age[index] =
								catch_biomass_at_age[index] / total_c_b;
						catch_biomass_proportion_at_age_males[index] =
								catch_biomass_at_age_males[index]
										/ total_c_b_males;
						catch_biomass_proportion_at_age_females[index] =
								catch_biomass_at_age_females[index]
										/ total_c_b_females;
	...
	void ComputeNLLComponents() {
		...
		for (int nllc = 0; nllc < this->nll_components.size(); nllc++) {
			...
			this->nll_components[nllc].Evaluate(
					this->nll_component_values[nllc]);
			this->nll += this->nll_component_values[nllc];
		}
	}

	void ApplyOperatingModelError() {
		this->catch_biomass_data = new mas::DataObject<REAL_T>();
		...
		this->catch_biomass_data->observation_error.resize(
				this->years * this->seasons);

		this->catch_proportion_at_age_data = new mas::DataObject<REAL_T>();
		...
		this->catch_proportion_at_age_data->data.resize(
				this->years * this->seasons * this->ages);
		this->catch_proportion_at_age_data->sample_size.resize(
				this->years * this->seasons);
		...
		for (int y = 0; y < this->years; y++) {
			for (int s = 0; s < this->seasons; s++) {
				...
				this->catch_proportion_at_age_data->sample_size[y
						* this->seasons + s] = distribution(generator);
				std::vector<int> ret = mas::rmultinom(
						this->catch_proportion_at_age_data->sample_size[y * s
								+ s], probs);
				for (int a = 0; a < this->ages; a++) {
	...
	std::string NLLComponentsToString() {
		...
		for (int nllc = 0; nllc < this->nll_components.size(); nllc++) {
			ss << std::setw(15) << this->nll_component_values[nllc].GetName()
					<< ":" << this->nll_component_values[nllc] << std::endl;
	...
	inline void EvaluateBiomassComponent(int year, int season) {
		...
		if (temp != catch_biomass_data->missing_value) {
			this->catch_biomass_component += .5
					* mas::pow(
							std::log(temp + o)
									- mas::log(
											this->catch_biomass_total[year
													* seasons + season] + o),
							2.0) / .05;
		}
	}
...
#warning hardcoded fishery age comp sample size
		...
		if (temp_sample_size
				!= this->catch_proportion_at_age_data->missing_value) {
			for (int a = 0; a < this->ages; a++) {
				REAL_T temp_data = this->catch_proportion_at_age_data->get(year,
						season, a);
				...
				this->fishery_age_comp_component -= temp_sample_size * temp_data
						* (mas::log(this->catch_proportion_at_age[index] + o)
								- std::log(temp_data + o));
			}
		}
	}
	...
	virtual const std::string ToJSONString() {
		...
		for (it = this->area_season_fishing_mortality_ids.begin();
				it != this->area_season_fishing_mortality_ids.end(); ++it) {
			ss << "{\n";
		...
		ss << "\"selectivity\" : [\n";
		for (it = this->area_season_selectivity_ids.begin();
				it != this->area_season_selectivity_ids.end(); ++it) {
			ss << "{\n";
			typename std::unordered_map<int, int>::iterator jt;
			int area = (*it).first;
			for (jt = (*it).second.begin(); jt != (*it).second.end(); ++jt) {
...
std::ostream& operator<<(std::ostream &out, const mas::Fleet<REAL_T> &fleet) {
	...
	for (int a = 0; a < fleet.ages; a++) {
		for (int y = 0; y < fleet.years; y++) {
			for (int s = 0; s < fleet.seasons; s++) {
				out
						<< fleet.catch_at_age[y * fleet.seasons * fleet.ages
								+ (s) * fleet.ages + a] << " ";
			}
		}
		out << "\n";
	...
	for (int a = 0; a < fleet.ages; a++) {
		for (int y = 0; y < fleet.years; y++) {
			for (int s = 0; s < fleet.seasons; s++) {
				out
						<< fleet.catch_at_age_males[y * fleet.seasons
								* fleet.ages + (s) * fleet.ages + a] << " ";
			}
		}
		out << "\n";
	}
	...
	for (int a = 0; a < fleet.ages; a++) {
		for (int y = 0; y < fleet.years; y++) {
			for (int s = 0; s < fleet.seasons; s++) {
				out
						<< fleet.catch_proportion_at_age[y * fleet.seasons
								* fleet.ages + (s) * fleet.ages + a] << " ";
			}
		}
		out << "\n";
	...
	for (int a = 0; a < fleet.ages; a++) {
		for (int y = 0; y < fleet.years; y++) {
			for (int s = 0; s < fleet.seasons; s++) {
				out
						<< fleet.catch_proportion_at_age_males[y * fleet.seasons
								* fleet.ages + (s) * fleet.ages + a] << " ";
			}
		}
		out << "\n";
	...
	for (int a = 0; a < fleet.ages; a++) {
		for (int y = 0; y < fleet.years; y++) {
			for (int s = 0; s < fleet.seasons; s++) {
				out
						<< fleet.catch_proportion_at_age_females[y
								* fleet.seasons * fleet.ages + (s) * fleet.ages
								+ a] << " ";
			}
		}
		out << "\n";
	...
	for (int a = 0; a < fleet.ages; a++) {
		for (int y = 0; y < fleet.years; y++) {
			for (int s = 0; s < fleet.seasons; s++) {
				out
						<< fleet.catch_biomass_at_age_males[y * fleet.seasons
								* fleet.ages + (s) * fleet.ages + a] << " ";
			}
		}
		out << "\n";
	...
	for (int a = 0; a < fleet.ages; a++) {
		for (int y = 0; y < fleet.years; y++) {
			for (int s = 0; s < fleet.seasons; s++) {
				out
						<< fleet.catch_biomass_at_age_females[y * fleet.seasons
								* fleet.ages + (s) * fleet.ages + a] << " ";
			}
		}
		out << "\n";
