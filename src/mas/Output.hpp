...
namespace mas {
    ...
    class JSONOutputGenerator {
        ...
        void GeneratePopulationRecruitment(rapidjson::Value &popobject,
            ...
            this->GenerateArrayObject(obj2, popinfo.recruitment_variance, 2, popinfo.years,
                    popinfo.seasons, popinfo.ages.size());
        ...
        void GeneratePopulationBiomassTotal(rapidjson::Value &popobject,
            ...
            rapidjson::Value obj2(rapidjson::kArrayType);
            this->GenerateArrayObject(obj2, popinfo.biomass_variance, 2, popinfo.years,
                    popinfo.seasons, popinfo.ages.size());
            popobject.AddMember("variance", obj2, this->document.GetAllocator());
        }

        void GeneratePopulationF_over_Fmsy(rapidjson::Value &popobject,
                const mas::Subpopulation<REAL_T> &popinfo) {
            rapidjson::Value obj(rapidjson::kArrayType);
            this->GenerateArrayObject(obj, popinfo.F_over_F_msy, 2, popinfo.years,
                    popinfo.seasons, popinfo.ages.size());
            popobject.AddMember("values", obj, this->document.GetAllocator());

            rapidjson::Value obj2(rapidjson::kArrayType);
            this->GenerateArrayObject(obj2, popinfo.F_over_F_msy_variance, 2, popinfo.years,
                    popinfo.seasons, popinfo.ages.size());
            popobject.AddMember("variance", obj2, this->document.GetAllocator());
        }
        ...
        const std::string GenerateOutput(mas::MAS<REAL_T> &mas) {
            ...
            for (int i = 0; i < mas.info.estimated_parameters.size(); i++) {
                ...
                if (std::fabs(g) > max_gc) {
                    ...
                    name_max_gc = n;
                    gc = g;
                    p = mas.info.estimated_parameters[i]->GetValue();
                }
            ...
            for (pit = info->populations.begin(); pit != info->populations.end();
                ...
                for (int i = 0; i < (*pit).second->areas_list.size(); i++) {
                    ...
                    rapidjson::Value ffoverfmsy(rapidjson::kObjectType);
                    this->GeneratePopulationF_over_Fmsy(ffoverfmsy,
                            (*pit).second->females[(*pit).second->areas_list[i]->id]);
                    females.AddMember("F_over_F_msy", ffoverfmsy, allocator);
                    ...
                    rapidjson::Value mfoverfmsy(rapidjson::kObjectType);
                    this->GeneratePopulationF_over_Fmsy(mfoverfmsy,
                            (*pit).second->males[(*pit).second->areas_list[i]->id]);
                    males.AddMember("F_over_F_msy", mfoverfmsy, allocator);
