...
#warning switch to observed catch biomass
        ...
        inline void CalculateSurveyNumbersAtAge(int year, int season) {
            ...
            this->S_Index_Total[index2] = total_SI;
        }
    ...
    public:
        ...
        std::vector<REAL_T> survey_index_at_age;
        std::vector<REAL_T> survey_index;
        std::vector<REAL_T> catch_numbers_at_age;
        std::vector<REAL_T> catch_index_at_age;
        std::vector<REAL_T> catch_index;
        std::vector<REAL_T> recruits;
        std::vector<REAL_T> spawning_stock_biomass;
        std::vector<REAL_T> biomass;
        std::vector<REAL_T> numbers;

        //females
        std::vector<REAL_T> numbers_at_age_females;
        std::vector<REAL_T> survey_numbers_at_age_females;
        std::vector<REAL_T> survey_index_at_age_females;
        std::vector<REAL_T> survey_index_females;
        std::vector<REAL_T> catch_numbers_at_age_females;
        std::vector<REAL_T> catch_index_at_age_females;
        std::vector<REAL_T> catch_index_females;
        std::vector<REAL_T> recruits_females;
        std::vector<REAL_T> spawning_stock_biomass_females;
        std::vector<REAL_T> biomass_females;
        std::vector<REAL_T> numbers_females;

        //males
        std::vector<REAL_T> numbers_at_age_males;
        std::vector<REAL_T> survey_numbers_at_age_males;
        std::vector<REAL_T> survey_index_at_age_males;
        std::vector<REAL_T> survey_index_males;
        std::vector<REAL_T> catch_numbers_at_age_males;
        std::vector<REAL_T> catch_index_at_age_males;
        std::vector<REAL_T> catch_index_males;
        std::vector<REAL_T> recruits_males;
        std::vector<REAL_T> spawning_stock_biomass_males;
        std::vector<REAL_T> biomass_males;
        std::vector<REAL_T> numbers_males;
...
#warning need to move biomass also
        ...
        std::vector<variable> survey_index_at_age; 
        std::vector<variable> survey_index;
        std::vector<variable> catch_numbers_at_age;
        std::vector<variable> catch_index_at_age;
        std::vector<variable> catch_index;
        ...
        std::vector<variable> spawning_stock_biomass;
            ...
            survey_index_at_age.resize(this->years * this->seasons * this->ages);
            survey_index.resize(this->years * this->seasons);
            catch_numbers_at_age.resize(this->years * this->seasons * this->ages);
            catch_index_at_age.resize(this->years * this->seasons * this->ages);
            catch_index.resize(this->years * this->seasons);
            recruits.resize(this->years * this->seasons);
            spawning_stock_biomass.resize(this->years * this->seasons);
            biomass.resize(this->years * this->seasons);
            numbers.resize(this->years * this->seasons);
            ...
            survey_index_at_age_females.resize(this->years * this->seasons * this->ages);
            survey_index_females.resize(this->years * this->seasons);
            catch_numbers_at_age_females.resize(this->years * this->seasons * this->ages);
            catch_index_at_age_females.resize(this->years * this->seasons * this->ages);
            catch_index_females.resize(this->years * this->seasons);
            recruits_females.resize(this->years * this->seasons);
            spawning_stock_biomass_females.resize(this->years * this->seasons);
            biomass_females.resize(this->years * this->seasons);
            numbers_females.resize(this->years * this->seasons);
            ...
            survey_index_at_age_males.resize(this->years * this->seasons * this->ages);
            survey_index_males.resize(this->years * this->seasons);
            catch_numbers_at_age_males.resize(this->years * this->seasons * this->ages);
            catch_index_at_age_males.resize(this->years * this->seasons * this->ages);
            catch_index_males.resize(this->years * this->seasons);
            recruits_males.resize(this->years * this->seasons);
            spawning_stock_biomass_males.resize(this->years * this->seasons);
            biomass_males.resize(this->years * this->seasons);
            numbers_males.resize(this->years * this->seasons);
        }
        ...
        void Show() {
            ...
            for (int a = 0; a < this->ages; a++) {
                for (int y = 0; y < this->years; y++) {
                    for (int s = 1; s <= this->seasons; s++) {
                        std::cout << this->catch_index_at_age[y * this->seasons * this->ages + (s - 1) * this->ages + a] << " ";
                    }
                }
                std::cout << "\n";
            }
            ...
            for (int a = 0; a < this->ages; a++) {
                for (int y = 0; y < this->years; y++) {
                    for (int s = 1; s <= this->seasons; s++) {
                        ...
                        std::cout << this->survey_index_at_age[y * this->seasons * this->ages + (s - 1) * this->ages + a] << " ";
                    }
                }
                std::cout << "\n";
            }
        ...
        std::vector<REAL_T> numbers;
                    ...
                    for (int s = 1; s <= this->seasons; s++) {
                        for (int a = 0; a < this->ages; a++) {
                            ...
                            this->catch_numbers_at_age[index] +=
                                    males[areas_list[al]->id].C[index] +
                                    females[areas_list[al]->id].C[index];
                            ...
                            this->survey_numbers_at_age[index] +=
                                    males[areas_list[al]->id].SN[index] +
                                    females[areas_list[al]->id].SN[index];
                            this->survey_numbers_at_age_females[index] +=
                                    females[areas_list[al]->id].SN[index];
                            this->survey_numbers_at_age_males[index] +=
                                    males[areas_list[al]->id].SN[index];

                            this->survey_index_at_age[index] +=
                                    males[areas_list[al]->id].S_Index[index] +
                                    females[areas_list[al]->id].S_Index[index];
                            this->survey_index_at_age_females[index] +=
                                    females[areas_list[al]->id].S_Index[index];
                            this->survey_index_at_age_males[index] +=
                                    males[areas_list[al]->id].S_Index[index];
                        }
                    }
                }
            ...
            for (int al = 0; al < areas_list.size(); al++) {
                for (int y = 0; y < this->years; y++) {
                    for (int s = 1; s <= this->seasons; s++) {
                        size_t index = y * this->seasons * this->ages + (s - 1);

                        this->catch_index[index] +=
                                males[areas_list[al]->id].C_Biomass_Total[index] +
                                females[areas_list[al]->id].C_Biomass_Total[index];
                        this->catch_index_at_age_females[index] +=
                                females[areas_list[al]->id].C_Biomass_Total[index];
                        this->catch_index_males[index] +=
                                males[areas_list[al]->id].C_Biomass_Total[index];

                        this->survey_index[index] +=
                                males[areas_list[al]->id].S_Index_Total[index] +
                                females[areas_list[al]->id].S_Index_Total[index];
                        this->survey_index_females[index] +=
                                females[areas_list[al]->id].S_Index_Total[index];
                        this->survey_index_males[index] +=
                                males[areas_list[al]->id].S_Index_Total[index];
                    }
                }
            }
