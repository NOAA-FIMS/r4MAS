...
public:
    ...
    static void GenerateArrayObject(rapidjson::Document& document,
            ...
            const Rcpp::NumericVector& darray,
            int dimensions,
            size_t imax,
            size_t jmax,
            size_t kmax) {
    ...
    virtual void AddToMAS(mas::Information<double>& info) {
        ...
        if (this->estimate) {
            for (int y = 0; y < info.nyears; y++) {
                ...
                for (int s = 0; s < info.nseasons; s++) {
                    std::stringstream ss;
                    ss << "fishing_mortality[" << y << "][" << s << "]_" << this->id;
                    mas::VariableTrait<double>::SetName(fm->fishing_mortality[y][s], ss.str());
                    if (this->min != std::numeric_limits<double>::min()) {
                        mas::VariableTrait<double>::SetMinBoundary(fm->fishing_mortality[y][s], this->min);
                    }
                    if (this->max != std::numeric_limits<double>::max()) {

                        mas::VariableTrait<double>::SetMaxBoundary(fm->fishing_mortality[y][s], this->max);
                    }
                    std::cout << "Registering " << ss.str() << " " << this->phase << "\n";
        ...
        if (fit != info.fishing_mortality_models.end()) {
            ...
            for (int y = 0; y < info.nyears; y++) {
                for (int s = 0; s < info.nseasons; s++) {
                    ...
                    this->values[i++] = (*fit).second->fishing_mortality[y][s].GetValue();
                }
            }
        }
    }
    ...
    virtual void AddToMAS(mas::Information<double>& info) {
        ...
        if (this->estimate) {
            for (int i = 0; i < info.ages.size(); i++) {
                ...
                mas::VariableTrait<double>::SetName(m->mortality_vector[i], ss.str());
                if (min != std::numeric_limits<double>::min()) {
                    mas::VariableTrait<double>::SetMinBoundary(m->mortality_vector[i], min);
                }
                if (max != std::numeric_limits<double>::max()) {

                    mas::VariableTrait<double>::SetMaxBoundary(m->mortality_vector[i], max);
                }
    ...
    virtual void AddToMAS(mas::Information<double>& info) {
        ...
        if (this->R0.estimated) {
            ...
            if (this->R0.max != std::numeric_limits<double>::max()) {
                mas::VariableTrait<double>::SetMaxBoundary(r->log_R0, std::log(this->R0.max));
            }
        ...
        if (this->estimate_deviations) {
            for (int i = 0; i < this->deviations.size(); i++) {
                ...
                mas::VariableTrait<double>::SetName(r->recruitment_deviations[i], ss.str());
                if (this->deviations_min != std::numeric_limits<double>::min()) {
                    mas::VariableTrait<double>::SetMinBoundary(r->recruitment_deviations[i], this->deviations_min);
                }
                if (this->deviations_max != std::numeric_limits<double>::max()) {
    ...
    virtual void AddToMAS(mas::Information<double>& info) {
        ...
        if (this->sigma_r.estimated) {
            ...
            if (this->sigma_r.min != std::numeric_limits<double>::min()) {
                mas::VariableTrait<double>::SetMinBoundary(r->sigma_r, sigma_r.min);
            }
            if (this->sigma_r.max != std::numeric_limits<double>::max()) {
                mas::VariableTrait<double>::SetMaxBoundary(r->sigma_r, sigma_r.max);
            }
...
public:
        ...
        if (this->has_emprical_weight == true) {
            ...
            if (this->weight_at_catch_females.size()) {
                if (this->weight_at_catch_females.size() != data_length) {
                    std::cout << "weight_at_catch_females vector not equal to (nyears*nseasons*nages)\n";
                    info.valid_configuration = false;
                    return;
                }
            ...
            if (this->weight_at_survey_females.size()) {
                if (this->weight_at_survey_females.size() != data_length) {
                    std::cout << "weight_at_survey_females vector not equal to (nyears*nseasons*nages)\n";
                    info.valid_configuration = false;
                    return;
                }
                size_t ages = info.ages.size();
            ...
            if (this->weight_at_spawning_females.size()) {
                if (this->weight_at_spawning_females.size() != data_length) {
                    std::cout << "weight_at_spawning_females vector not equal to (nyears*nseasons*nages)\n";
                    info.valid_configuration = false;
                    return;
                }
                size_t ages = info.ages.size();
            ...
            if (this->weight_at_season_start_females.size()) {
                if (this->weight_at_season_start_females.size() != data_length) {
                    std::cout << "weight_at_season_start_females vector not equal to (nyears*nseasons*nages)\n";
                    info.valid_configuration = false;
                    return;
                }
                size_t ages = info.ages.size();
    ...
    virtual void ToJSON(rapidjson::Document& document, size_t nyears, size_t nseasons, size_t nages, size_t nareas) {
        ...
        parameters.AddMember("l_inf", linf, allocator);
            ...
            for (int s = 0; s < info.nseasons; s++) {
                for (int i = 0; i < Area::id_g; i++) {
                    for (int j = 0; j < Area::id_g; j++) {
                        ...
                        this->connectivity_males[k] = m->male_connectivity[s][i][j].GetValue();
                        this->connectivity_females[k] = m->female_connectivity[s][i][j].GetValue();
                        this->connectivity_recruits[k++] = m->recruit_connectivity[s][i][j].GetValue();
                    }
                }
            }
        }
            ...
            if (dit != InitialDeviations::initialized_models.end()) {
                ...
                if (devs->values.size() != info.ages.size()) {
                    ...
                    if (devs->estimate) {
                        for (int i = 0; i < v.size(); i++) {
                            std::stringstream ss;
                            ss << "f_initial_deviations[" << i << "]_" << pop->id;
            ...
            if (dit != InitialDeviations::initialized_models.end()) {
                ...
                if (devs->values.size() != info.ages.size()) {
                    ...
                    if (devs->estimate) {
                        for (int i = 0; i < v.size(); i++) {
                            std::stringstream ss;
                            ss << "m_initial_deviations[" << i << "]_" << pop->id;
                            mas::VariableTrait<double>::SetName(v[i], ss.str());
                            pop->Register(v[i], devs->phase);
                        }
                    }
