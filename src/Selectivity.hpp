...
namespace mas {
    ...
    struct SelectivityBase : mas::ModelObject<REAL_T> {
        ...
        virtual variable LikelihoodComponent(int phase) {
            ...
            //            assert(this->initial_parameter_values.size() == this->lambdas.size());
...
//                    if (p > 0.0) {
                        ret += lambdas[i] * (atl::log(sigma) + 0.5 * SELX_SQUARE(std::log(this->initial_parameter_values[i]) - atl::log(p)) / sigma2);
//                    } else {
